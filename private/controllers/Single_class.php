...
class Single_class extends Controller
{
	...
	public function lectureradd($id ='')
	{
		...
		if(count($_POST) > 0)
		{
			...
			{
				$query = "select id from class_lecturers where user_id = :user_id && class_id = :class_id && disabled = 1 limit 1";
				$queryactive = "select id from class_lecturers where user_id = :user_id && class_id = :class_id && disabled = 0 limit 1";

				//add lecturer
				// Before adding, first check if the lecturer is not disabled
				if(!$lect->query($query, [
					'user_id' => $_POST['selected'],
					'class_id' => $id,
				]))
				{
					// secondly check if the lecturer is not active
					if(!$lect->query($queryactive, [
						'user_id' => $_POST['selected'],
						'class_id' => $id,
					]))
					{
					...
					// check class_lecturer table for lecturers with disabled 1 and update it to 0 when you try to add the lecturer again
				}elseif ($row = $lect->query($query, [
						'user_id' => $_POST['selected'],
						'class_id' => $id,
					])) {
						$arr = array();
	...
	public function studentadd($id ='')
	{
		...
		if(count($_POST) > 0)
		{
			...
			{
				$query = "select id from class_students where user_id = :user_id && class_id = :class_id && disabled = 1 limit 1";
				$queryactive = "select id from class_students where user_id = :user_id && class_id = :class_id && disabled = 0 limit 1";

				//add student
				// Before adding, first check if the student is not disabled
				if(!$stud->query($query, [
					'user_id' => $_POST['selected'],
					'class_id' => $id,
				]))
				{
					// secondly check if the student is not active
					if(!$stud->query($queryactive, [
						'user_id' => $_POST['selected'],
						'class_id' => $id,
					]))
					{
					...
					// check class_student table for students with disabled 1 and update it to 0 when you try to add the student again
				}elseif ($row = $stud->query($query, [
						'user_id' => $_POST['selected'],
						'class_id' => $id,
					])) {
						$arr = array();
